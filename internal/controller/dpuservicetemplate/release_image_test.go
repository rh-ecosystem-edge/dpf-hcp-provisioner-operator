...
var _ = Describe("Release Image Utilities", func() {
	...
	Describe("splitImage", func() {
		...
		It("should return error when no tag or digest present", func() {
			_, _, err := splitImage("quay.io/openshift/ovn")
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("has no tag or digest"))
		})
