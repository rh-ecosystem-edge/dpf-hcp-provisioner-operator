...
var _ = Describe("DPFHCPProvisioner Phase Transitions", func() {
	...
	Context("Controller-Driven Phase Computation", func() {
		...
		It("isHostedClusterVersionCompleted should return true when version matches and state is Completed", func() {
			reconciler := &DPFHCPProvisionerReconciler{}
			...
			Expect(reconciler.isHostedClusterVersionCompleted(hc, "quay.io/openshift-release-dev/ocp-release:4.22.1-multi")).To(BeTrue())
		})

		It("isHostedClusterVersionCompleted should return false when version matches but state is Partial", func() {
			reconciler := &DPFHCPProvisionerReconciler{}
			hc := &hyperv1.HostedCluster{
				Status: hyperv1.HostedClusterStatus{
					Version: &hyperv1.ClusterVersionStatus{
			...
			Expect(reconciler.isHostedClusterVersionCompleted(hc, "quay.io/openshift-release-dev/ocp-release:4.22.1-multi")).To(BeFalse())
		})

		It("isHostedClusterVersionCompleted should return false when version does not match", func() {
			reconciler := &DPFHCPProvisionerReconciler{}
			hc := &hyperv1.HostedCluster{
				Status: hyperv1.HostedClusterStatus{
					Version: &hyperv1.ClusterVersionStatus{
		...
		It("isHostedClusterVersionCompleted should return false when version history is empty", func() {
			reconciler := &DPFHCPProvisionerReconciler{}
			hc := &hyperv1.HostedCluster{
				Status: hyperv1.HostedClusterStatus{
					Version: &hyperv1.ClusterVersionStatus{},
		...
		It("isHostedClusterVersionCompleted should return false when version is nil", func() {
			reconciler := &DPFHCPProvisionerReconciler{}
			hc := &hyperv1.HostedCluster{}
			Expect(reconciler.isHostedClusterVersionCompleted(hc, "quay.io/openshift-release-dev/ocp-release:4.22.1-multi")).To(BeFalse())
		})
