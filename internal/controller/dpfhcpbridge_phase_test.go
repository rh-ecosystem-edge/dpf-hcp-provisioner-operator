	...
	Context("Controller-Driven Phase Computation", func() {
		It("should transition to Pending when all validations pass", func() {
			...
			Eventually(func() bool {
				...
				dpuClusterMissingCond := meta.FindStatusCondition(bridge.Status.Conditions, provisioningv1alpha1.DPUClusterMissing)
				...
				clusterTypeValidCond := meta.FindStatusCondition(bridge.Status.Conditions, provisioningv1alpha1.ClusterTypeValid)
				...
				dpuClusterInUseCond := meta.FindStatusCondition(bridge.Status.Conditions, provisioningv1alpha1.DPUClusterInUse)
				...
				secretsCond := meta.FindStatusCondition(bridge.Status.Conditions, provisioningv1alpha1.SecretsValid)
				...
				imageCond := meta.FindStatusCondition(bridge.Status.Conditions, provisioningv1alpha1.BlueFieldImageResolved)
