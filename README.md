...
- **DPUServiceTemplate management** -- creates OVN, DTS, and HBN DPUServiceTemplate resources in every namespace referenced by a `DPFHCPProvisioner`'s `DPUClusterRef`, with chart versions selected based on the DPF operator version. Must be enabled through `manageDPUServiceTemplates` in `DPFHCPProvisionerConfig`
