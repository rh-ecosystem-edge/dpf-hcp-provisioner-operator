...
```bash
kubectl patch dpfhcpconfig default --type merge -p '{"spec":{"blueFieldOCPLayerRepo":"quay.io/<user>/bluefield-ocp-layer"}}'
```
...
|-------|------|---------|-------------|
| `spec.blueFieldOCPLayerRepo` | `string` | `registry.redhat.io/dpu-kit-for-nvidia/bluefield-ocp-layer-rhel10` | Container registry repository for BlueField RHCOS OCP image layers. The operator queries this repo to find an image tag matching the OCP version. The lookup is skipped when `machineOSURL` is provided in the DPFHCPProvisioner spec. |
